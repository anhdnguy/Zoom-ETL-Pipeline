...
module "webserver" {
  ...
  health_check_command = [
    "CMD-SHELL",
    "python -c 'import urllib.request; urllib.request.urlopen(\"http://localhost:8080/api/v2/version\")' || exit 1"
  ]
...
module "scheduler" {
  ...
  dns_name       = module.alb.dns_name
  container_port = 8974 # Scheduler health check port

  health_check_command = [
    "CMD-SHELL",
    "airflow jobs check --job-type SchedulerJob || exit 1"
  ]
...
module "triggerer" {
  ...
  health_check_command = [
    "CMD-SHELL",
    "airflow jobs check --job-type TriggererJob || exit 1"
  ]
  health_check_start_period = 180
}
...
module "worker" {
  ...
  dns_name       = module.alb.dns_name
  container_port = 8793 # Worker log-serving port

  health_check_command = [
    "CMD-SHELL",
    "airflow celery list-workers || exit 1"
  ]
  health_check_start_period = 180
}
